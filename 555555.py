# -*- coding: utf-8 -*-
"""
Versión con nombres de meses en log y limpieza de columnas vacías en 'Registro'.
"""
...
class LogWindow(tk.Tk):
    ...
    def add_files(self):
        ...
        self.log(f"🧾 Total de archivos seleccionados: {len(self.selected_files)}")
    ...
    def export_results(self):
        ...
        dest = asksaveasfilename(
            title="Guardar archivo de resumen",
        ...
        if not dest:
            return
        try:
            shutil.copy("log.txt", Path(dest).with_name("log.txt"))
            ...
            self.log(f"✅ Exportado: {Path(dest).name} + log.txt")
...
def main_process(log_ui, selected_files):
    try:
        ...
        years = sorted({int(pd.read_excel(f, engine="openpyxl").iloc[1,11]) for f in selected_files})
