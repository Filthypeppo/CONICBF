# -*- coding: utf-8 -*-
"""
Versión: log en memoria + creación/escritura de log.txt solo al exportar (sobreescribe si existe)
"""
...
class LogWindow(tk.Tk):
    def __init__(self):
        ...
        # --- log en memoria (lista de líneas) ---
        self.log_lines = []
    ...
    def log(self, msg):
        """Añade línea al log en pantalla y a buffer en memoria (no escribe archivo hasta export)."""
    ...
    def load_new_files(self):
        """Limpia selección previa y pide archivos nuevos."""
    ...
    def add_files(self):
        ...
        if not files:
            self.log("No se seleccionaron archivos.")
            return
    ...
    def run_analysis(self):
        if not self.selected_files:
            ...
            return
        # vaciar log en memoria antes de procesar si quieres empezar clean
        # aquí mantenemos log previo y añadimos nuevas líneas; si prefieres limpiar, descomenta la siguiente línea:
        # self.log_lines.clear(); self.text.delete("1.0","end")
        self.after(100, lambda: main_process(self, self.selected_files))

    def export_results(self):
        """Pide nombre y carpeta, escribe el Excel y luego escribe (o sobrescribe) log.txt en la misma carpeta."""
        ...
        if not dest:
            ...
            return
        try:
            # --- limpiar columnas vacías antes de exportar ---
            ...
            # Escribir log.txt en la misma carpeta (sobreescribiendo si existe)
            ...
            self.log(f"✅ Exportado: {Path(dest).name} y log.txt (guardado en {dest_path})")
            showinfo("Exportación completada", f"Archivos exportados correctamente:\n{dest}\n{log_path}")
...
def analyze(selected_files, holidays_freq, log_ui):
    ...
    # Agrupar sumando columnas numéricas; mantener solo columnas con nombre
    if Rg.empty:
        return None, None, None, all_errors

    # eliminar columnas sin nombre (None o "")
    ...
    # limpiar columnas vacías en Omega también
...
def main_process(log_ui, selected_files):
    try:
        ...
        # Determinar años a consultar a partir de los archivos (ignorando archivos corruptos)
        years = set()
